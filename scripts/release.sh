...
get_version() {
  ...
  # 新しいバージョンの入力を求める
  read -p "新しいバージョン番号を入力してください（例: 1.0.0）: " new_version
...
create_release_pr() {
  ...
  if [ -d "pr-body" ] && [ -f "pr-body/release-pr-body.md" ]; then
    ...
    # GitHub Releaseの作成
    if command -v gh &> /dev/null; then
      gh release create "v$new_version" --title "v$new_version" --notes "リリースノートについてはchanges.mdを参照してください。"
      info "GitHub Releaseを作成しました。"
    else
      info "変更をプッシュしました。GitHub Releaseは手動で作成してください。"
    fi
  fi
}
